...
const defaults={owned:['neon'],equipped:{}};
...
const render=()=>{const s=load(),w=window.VVEconomy?.load?.()||{coins:0,gems:0};const coins=document.getElementById('customCoins'),gems=document.getElementById('customGems'),list=document.getElementById('customList');if(coins)coins.textContent=w.coins.toLocaleString();if(gems)gems.textContent=w.gems;if(!list)return;list.innerHTML=items.map(x=>{const owned=s.owned.includes(x.id),eq=s.equipped[x.type]===x.id;return `<button class="custom-item ${eq?'equipped':''}" data-custom="${x.id}" style="--a:${x.unit==='gems'?'#ffb52e':'#37d9ff'}"><div class="custom-swatch"></div><b>${x.name}</b><span>${x.type} • ${eq?'EQUIPPED':owned?'OWNED':x.cost+' '+x.unit.toUpperCase()}</span><strong>${eq?'✓ EQUIPPED':owned?'EQUIP':'UNLOCK'}</strong></button>`}).join('')};
const handle=e=>{const b=e.target.closest('[data-custom]');if(!b)return;const id=b.dataset.custom,s=load(),x=items.find(i=>i.id===id);if(!x)return;if(s.owned.includes(id)){s.equipped[x.type]=id;save(s);const note=document.getElementById('customNote');if(note)note.textContent=x.name.toUpperCase()+' EQUIPPED';render();return}if(buy(x,s)){s.equipped[x.type]=id;save(s);const note=document.getElementById('customNote');if(note)note.textContent=x.name.toUpperCase()+' UNLOCKED AND EQUIPPED';render()}else{const note=document.getElementById('customNote');if(note)note.textContent='NOT ENOUGH '+x.unit.toUpperCase()+' TO UNLOCK THIS ITEM.'}};
const open=()=>{let m=document.getElementById('customModal');if(!m){m=document.createElement('div');m.id='customModal';m.className='modal';m.innerHTML='<div class="modal-panel custom-panel"><button class="modal-close" id="customClose">×</button><span class="eyebrow">CUSTOMIZATION</span><h2>MAKE IT YOURS</h2><p>Cosmetics change style, not competitive power.</p><div class="custom-wallet"><b>◈ <span id="customCoins">0</span> COINS</b><b>◆ <span id="customGems">0</span> GEMS</b></div><div id="customList" class="custom-grid"></div><p id="customNote" class="custom-note">Select a cosmetic to equip it or unlock it with your wallet.</p><button class="primary" id="customBack">BACK</button></div>';document.body.appendChild(m);document.getElementById('customClose').onclick=()=>m.classList.add('hidden');document.getElementById('customBack').onclick=()=>m.classList.add('hidden');document.getElementById('customList').addEventListener('click',handle)}render();m.classList.remove('hidden')};
