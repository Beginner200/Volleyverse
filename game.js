...
window.addEventListener('vv-online-session',e=>{const d=e.detail||{};onlineMatchActive=true;localSessionStarted=true;onlineTeam=d.localTeam||'home';onlineSlot=Number.isInteger(d.localSlot)?d.localSlot:-1;localStorage.setItem('volleyverseMatchMode','online');setTimeout(()=>{if(!state.ready)createScene();tip('ONLINE 6V6 • LIVE MATCH');},0)});window.VVOnlineClient?.on?.('point',m=>{const w=m?.winner==='home'?'HOME':'AWAY';if(m?.score){homeScore=Number(m.score.home)||0;awayScore=Number(m.score.away)||0}if(m?.sets){homeSets=Number(m.sets.home)||0;awaySets=Number(m.sets.away)||0}if(m?.setNumber)setNumber=Number(m.setNumber)||setNumber;updateHUD();tip('REFEREE • POINT '+w+(m?.reason?' • '+String(m.reason).replaceAll('_',' '):''));});window.VVOnlineClient?.on?.('matchReward',m=>{const reward={won:!!m?.won,coins:Number(m?.coins)||0,gems:Number(m?.gems)||0,careerXP:Number(m?.careerXP)||0,seasonXP:Number(m?.seasonXP)||0,battlePassXP:Number(m?.battlePassXP)||0};window.VVMatchRewards?.openSummary?.(reward);});window.VVOnlineClient?.on?.('matchReward',()=>{setTimeout(()=>window.VVServerProgression?.pull?.(),250);});window.VVOnlineClient?.on?.('matchFinished',()=>{setTimeout(()=>window.VVServerProgression?.pull?.(),500);});window.VVOnlineClient?.on?.('matchFinished',m=>{onlineMatchActive=false;rallyLocked=true;if(m?.sets){homeSets=Number(m.sets.home)||homeSets;awaySets=Number(m.sets.away)||awaySets}const won=m?.winner==='home';tip('MATCH COMPLETE • '+(won?'VICTORY':'DEFEAT'));updateHUD();const old=document.getElementById('onlineResults');if(old)old.remove();const box=document.createElement('div');box.id='onlineResults';box.className='modal';const hp=m?.sets?.home??homeSets,ap=m?.sets?.away??awaySets;box.innerHTML='<div class="modal-panel online-results"><span class="eyebrow">ONLINE 6V6 • FINAL RESULT</span><h2>'+ (won?'VICTORY':'DEFEAT') +'</h2><div class="online-final-score"><div><b>'+hp+'</b><span>VERSE SETS</span></div><strong>—</strong><div><b>'+ap+'</b><span>RIVALS SETS</span></div></div><p>Server-authoritative match complete. All players received the same final result.</p><div class="mm-actions"><button class="primary" id="onlineResultsClose">RETURN TO MENU</button></div></div>';document.body.appendChild(box);document.getElementById('onlineResultsClose').onclick=()=>{box.remove();location.reload()};});window.addEventListener('vv-online-snapshot',e=>{if(onlineMatchActive)applyOnlineSnapshot(e.detail||{})});
