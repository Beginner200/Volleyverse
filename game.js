...
window.addEventListener('vv-online-session',e=>{const d=e.detail||{};onlineMatchActive=true;localSessionStarted=true;onlineTeam=d.localTeam||'home';onlineSlot=Number.isInteger(d.localSlot)?d.localSlot:-1;localStorage.setItem('volleyverseMatchMode','online');setTimeout(()=>{if(!state.ready)createScene();tip('ONLINE 6V6 • LIVE MATCH');},0)});window.VVOnlineClient?.on?.('point',m=>{const w=m?.winner==='home'?'HOME':'AWAY';if(m?.score){homeScore=Number(m.score.home)||0;awayScore=Number(m.score.away)||0}if(m?.sets){homeSets=Number(m.sets.home)||0;awaySets=Number(m.sets.away)||0}if(m?.setNumber)setNumber=Number(m.setNumber)||setNumber;updateHUD();tip('REFEREE • POINT '+w+(m?.reason?' • '+String(m.reason).replaceAll('_',' '):''));});window.VVOnlineClient?.on?.('matchReward',m=>{const reward={won:!!m?.won,coins:Number(m?.coins)||0,gems:Number(m?.gems)||0,careerXP:Number(m?.careerXP)||0,seasonXP:Number(m?.seasonXP)||0,battlePassXP:Number(m?.battlePassXP)||0};window.VVMatchRewards?.openSummary?.(reward);});window.VVOnlineClient?.on?.('matchReward',()=>{setTimeout(()=>window.VVServerProgression?.pull?.(),250);});window.VVOnlineClient?.on?.('matchFinished',()=>{setTimeout(()=>window.VVServerProgression?.pull?.(),500);});window.VVOnlineClient?.on?.('matchFinished',m=>{
  onlineMatchActive=false;rallyLocked=true;
  if(m?.sets){homeSets=Number(m.sets.home)||homeSets;awaySets=Number(m.sets.away)||awaySets}
  const won=m?.winner==='home';tip('MATCH COMPLETE • '+(won?'VICTORY':'DEFEAT'));updateHUD();
  const old=document.getElementById('onlineResults');if(old)old.remove();
  const box=document.createElement('div');box.id='onlineResults';box.className='modal';
  const hp=m?.sets?.home??homeSets,ap=m?.sets?.away??awaySets;
  const points=m?.totalPoints||{home:0,away:0};const me=(m?.playerStats||[]).find(p=>String(p.id||'').startsWith(String(window.VVOnlineClient?.getLocalPlayer?.()?.accountId||'')+'_')||Number(p.slot)===Number(onlineSlot)&&p.team===onlineTeam);const perf=me?.stats?{...me.stats,grade:Math.max(0,Math.min(100,Math.round(50+((me.stats.serve||0)+(me.stats.receive||0)+(me.stats.set||0)+(me.stats.spike||0)+(me.stats.block||0)+(me.stats.dig||0))*4-((me.stats.errors||0)*5))))}:onlinePerformanceStats();const mvp=m?.mvp||null;const myPlayerId=window.VVOnlineClient?.getLocalPlayer?.()?.slot;const isMvp=mvp&&Number(mvp.slot)===Number(myPlayerId)&&mvp.team===onlineTeam;
  const localPoints=onlineTeam==='away'?Number(points.away)||0:Number(points.home)||0;
  const opponentPoints=onlineTeam==='away'?Number(points.home)||0:Number(points.away)||0;
  box.innerHTML='<div class="modal-panel online-results"><span class="eyebrow">ONLINE 6V6 • MATCH REPORT</span><h2>'+ (won?'VICTORY':'DEFEAT') +'</h2><div class="online-final-score"><div><b>'+hp+'</b><span>HOME SETS</span></div><strong>—</strong><div><b>'+ap+'</b><span>AWAY SETS</span></div></div><div class="online-stat-grid"><div><b>'+localPoints+'</b><span>YOUR TEAM POINTS</span></div><div><b>'+opponentPoints+'</b><span>OPPONENT POINTS</span></div><div><b>'+((window.VVOnlineClient?.getStatus?.()||{}).pingMs||0)+' ms</b><span>FINAL PING</span></div></div><h3 class="online-perf-title">YOUR PERFORMANCE</h3><div class="online-mvp-badge">'+(isMvp?'★ SERVER MVP • +25 CAREER XP • +50 SEASON XP • +75 BATTLE PASS XP':'SERVER-VERIFIED PERFORMANCE')+'</div><div class="online-performance-grid"><div><b>'+perf.serve+'</b><span>SERVES</span></div><div><b>'+perf.receive+'</b><span>RECEIVES</span></div><div><b>'+perf.set+'</b><span>SETS</span></div><div><b>'+perf.spike+'</b><span>SPIKES</span></div><div><b>'+perf.block+'</b><span>BLOCKS</span></div><div><b>'+perf.dig+'</b><span>DIGS</span></div><div><b>'+perf.errors+'</b><span>ERRORS</span></div><div><b>'+perf.grade+'</b><span>MATCH GRADE</span></div></div></div><p>Final result and competitive outcome were confirmed by the VOLLEYVERSE server.</p><div class="mm-actions"><button class="primary" id="onlineResultsClose">RETURN TO MENU</button></div></div>';
  document.body.appendChild(box);document.getElementById('onlineResultsClose').onclick=()=>{box.remove();location.reload()};
});window.addEventListener('vv-online-snapshot',e=>{if(onlineMatchActive)applyOnlineSnapshot(e.detail||{})});
...
function onlinePerformanceStats(){const base={serve:0,receive:0,set:0,spike:0,block:0,dig:0,errors:0};try{const raw=JSON.parse(localStorage.getItem('volleyversePerformance')||'null');if(raw?.stats)Object.keys(base).forEach(k=>base[k]=Math.max(0,Number(raw.stats[k])||0))}catch(e){}const positive=base.serve+base.receive+base.set+base.spike+base.block+base.dig;const grade=Math.max(0,Math.min(100,Math.round(50+(positive*4)-(base.errors*5))));return {...base,grade}}
