...
function serve(){if(!state.ready||ballState.active||rallyLocked||servingTeam!=='home')return;ball.position.set(controlled.position.x,1.95,-4.15);ballState.v.set(THREE.MathUtils.clamp(controlled.position.x*.06,-.65,.65),5.8,7.2);ballState.active=true;ballState.lastTouch='home';ballState.side='home';ballState.touches=1;ballState.lastAction='serve';ballState.teamTouches={home:1,away:0};ballState.rallyPhase='IN_PLAY';ballState.serveTeam='home';controlled.userData.action=.55;tip('SERVE IN PLAY • MOVE INTO POSITION')}
...
function physics(dt){
  ...
  if(ball.position.y<.2){const landingTeam=ball.position.z<0?'home':'away';const winner=landingTeam===ballState.lastTouch?(landingTeam==='home'?'away':'home'):ballState.lastTouch;point(winner,'BALL DOWN');return}if(Math.abs(ball.position.z)>5.05||Math.abs(ball.position.x)>9.05){const last=ballState.lastTouch;point(last==='home'?'away':'home','OUT');return}
}
