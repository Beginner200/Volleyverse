...
function action(type){
  ...
  if(type==='pass'){const quality=skillQuality(controlled,'pass',.88);const setter=homePlayers.find(p=>p.userData.position==='S'||p.userData.position==='SETTER')||homePlayers[1];targetX=setter.position.x;launchTo(targetX+((Math.random()-.5)*(1-quality)*1.4),setter.position.z,2.2,5.8*quality);ballState.targetX=targetX;ballState.targetZ=setter.position.z;tip('PASS • FIRST TOUCH → SETTER')}
  else if(type==='set'){const quality=skillQuality(controlled,'set',.9);const attacker=homePlayers.find(p=>p.userData.position==='OH'||p.userData.position==='OPP')||homePlayers[2];targetX=attacker.position.x;launchTo(targetX+((Math.random()-.5)*(1-quality)*1.2),attacker.position.z,3.25,5.3*quality);ballState.targetX=targetX;ballState.targetZ=attacker.position.z;tip('SET • SECOND TOUCH → ATTACK')}
  else if(type==='spike'){const quality=skillQuality(controlled,'spike',.92);targetX=THREE.MathUtils.clamp(controlled.position.x+dx*.55,-4.0,4.0);const targetZ=THREE.MathUtils.clamp(4.0+Math.abs(controlled.position.z)*.02,3.65,4.2);launchTo(targetX+((Math.random()-.5)*(1-quality)*1.5),targetZ,.3,7.4*(.86+quality*.14));ballState.targetX=targetX;ballState.targetZ=targetZ;tip('SPIKE • THIRD TOUCH → ATTACK')}
  else if(type==='block'){if(ball.position.z>-.9&&ball.position.y>1.25){const quality=skillQuality(controlled,'block',.9);ballState.v.set(dx*.2*quality,5.0+quality*1.4,-Math.abs(ballState.v.z)*(.72+quality*.2)||-6.5);ballState.teamTouches.home=1;tip('BLOCK • CLOSE THE ANGLE')}else return}
  else if(type==='dive'){const quality=skillQuality(controlled,'dive',.86);controlled.position.z=THREE.MathUtils.clamp(controlled.position.z+.7,-4.35,-.3);launchTo(controlled.position.x+((Math.random()-.5)*(1-quality)*1.2),-2.2,2.0,4.8+quality*.9);ballState.targetX=controlled.position.x;ballState.targetZ=-2.2;tip('DIG • KEEP THE RALLY ALIVE')}
