...
function applyOnlineSnapshot(payload){const s=payload?.state||payload;if(!s)return;onlineMatchActive=true;localSessionStarted=true;onlineTeam=payload?.localTeam||onlineTeam;onlineSlot=Number.isInteger(payload?.localSlot)?payload.localSlot:onlineSlot;homeScore=s.scores?.home??homeScore;awayScore=s.scores?.away??awayScore;homeSets=s.sets?.home??homeSets;awaySets=s.sets?.away??awaySets;setNumber=s.setNumber??setNumber;servingTeam=s.serving||servingTeam;rallyLocked=s.status==='FINISHED';const all=[...homePlayers,...awayPlayers];if(Array.isArray(s.players)){s.players.forEach(v=>{const p=all[v.team==='home'?Number(v.slot):6+Number(v.slot)];if(!p)return;p.userData.onlineServerId=v.id;p.userData.moveX=THREE.MathUtils.clamp((Number(v.x)-p.position.x)*8,-1,1);p.userData.moveZ=THREE.MathUtils.clamp((Number(v.z)-p.position.z)*8,-1,1);p.position.set(Number(v.x)||0,Number(v.y)||0,Number(v.z)||0);p.userData.action=v.connected?0:.25;p.userData.onlineConnected=v.connected!==false;p.userData.aiTakeover=!!v.aiTakeover})}if(s.ball&&ball){ball.position.set(Number(s.ball.x)||0,Number(s.ball.y)||0,Number(s.ball.z)||0);ballState.v.set(Number(s.ball.vx)||0,Number(s.ball.vy)||0,Number(s.ball.vz)||0);ballState.active=!!s.rally?.active;ballState.lastTouch=s.ball.lastTeam||ballState.lastTouch;ballState.teamTouches=s.rally?.teamTouches||ballState.teamTouches;ballState.touches=Number(s.rally?.touches)||0}const mine=onlineTeam==='away'?awayPlayers[onlineSlot]:homePlayers[onlineSlot];if(mine){controlled=mine;[...homePlayers,...awayPlayers].forEach(p=>{p.userData.ring.visible=false;p.userData.arrow.visible=false});mine.userData.ring.visible=true;mine.userData.arrow.visible=true}updateHUD();tip(onlineTeam==='away'?'ONLINE 6V6 • AWAY':'ONLINE 6V6 • HOME')}
