...
let last=performance.now();function loop(now){requestAnimationFrame(loop);const dt=Math.min(Math.max((now-last)/1000,0),.025);last=now;if(state.ready){try{physics(dt);animatePlayers(dt);if(onlineMatchActive&&window.VVOnlineClient?.isConnected?.()&&now-onlineLastInputAt>70){const o=window.VVOnlineClient.getLocalPlayer?.()||{};if(Number.isInteger(o.slot)){onlineSlot=o.slot;window.VVOnlineClient.input(o.slot,keys.x,keys.z,null);onlineLastInputAt=now}}if(localConnected()&&localHost())window.VVLocalMultiplayer.sendSnapshot(localNetSnapshot());if(localStorage.getItem('volleyverseCamera')==='player'&&controlled){cameraFollowTarget.set(controlled.position.x+5.2,4.4,controlled.position.z+6.4);state.camera.position.lerp(cameraFollowTarget,.08);state.camera.lookAt(controlled.position.x,1,controlled.position.z)}if(now-state.lastRender>=state.renderEvery){state.renderer.render(state.scene,state.camera);state.lastRender=now;state.frameErrors=0}}catch(err){state.frameErrors++;console.error('VOLLEYVERSE frame error',err);if(state.frameErrors>=3){paused=true;tip('MATCH PAUSED • RECOVERING FROM A GRAPHICS ERROR');}}}requestAnimationFrame(loop)}
