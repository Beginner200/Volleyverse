...
function buildTeams(){
  ...
  homeBenchPlayers=homeBenchIds.map(id=>{const ch=roster.find(c=>c.id===id);const p=createPlayer(ch.color||0x36bfff,ch.name,5.8,6.8+homeBenchPlayers.length*.8,true,ch.position||'OH',matchStats(ch));p.userData.characterId=ch.id;p.userData.playerState.liberoEligible=ch.position==='L';p.visible=false;return p});
  const awayBenchIds=roster.map(c=>c.id).filter(id=>!awayIds.includes(id)&&id!==selectedLibero?.id).slice(0,3);
  awayBenchPlayers=awayBenchIds.map(id=>{const ch=roster.find(c=>c.id===id);const p=createPlayer(ch.color||0xff4f79,-5.8,6.8+awayBenchPlayers.length*.8,false,ch.position||'OH',matchStats(ch));p.userData.characterId=ch.id;p.userData.playerState.liberoEligible=ch.position==='L';p.visible=false;return p});
...
window.VVSubstitutionAPI={
 substitute:(team,inId,outId)=>{if(ballState.active||!rallyLocked)return{ok:false,reason:'SUBSTITUTION_ONLY_BETWEEN_RALLIES'};const result=VVPlayerState.substitute(team,inId,outId,{rallyNumber:VVPlayerState.rallyNumber});if(!result.ok)return result;const incoming=allTeamObjects(team).find(p=>p.userData.playerState.id===inId),outgoing=allTeamObjects(team).find(p=>p.userData.playerState.id===outId);if(incoming&&outgoing){incoming.position.copy(outgoing.position);incoming.userData.baseX=outgoing.userData.baseX;incoming.userData.baseZ=outgoing.userData.baseZ;incoming.userData.rotationPosition=result.rotationPosition}applyRosterChange(team);tip('SUBSTITUTION • '+result.incoming.name+' IN / '+result.outgoing.name+' OUT');return result},
 liberoIn:(team,liberoId,outId)=>{if(ballState.active||!rallyLocked)return{ok:false,reason:'LIBERO_ONLY_BETWEEN_RALLIES'};const result=VVPlayerState.replaceWithLibero(team,liberoId,outId,{rallyNumber:VVPlayerState.rallyNumber});if(!result.ok)return result;const libero=allTeamObjects(team).find(p=>p.userData.playerState.id===liberoId),outgoing=allTeamObjects(team).find(p=>p.userData.playerState.id===outId);if(libero&&outgoing){libero.position.copy(outgoing.position);libero.userData.baseX=outgoing.userData.baseX;libero.userData.baseZ=outgoing.userData.baseZ;libero.userData.rotationPosition=result.rotationPosition}applyRosterChange(team);tip('LIBERO IN • '+result.incoming.name+' FOR '+result.outgoing.name);return result},
 liberoOut:(team,liberoId)=>{if(ballState.active||!rallyLocked)return{ok:false,reason:'LIBERO_ONLY_BETWEEN_RALLIES'};const libero=VVPlayerState.get(liberoId),outId=libero?.lastReplacedId;if(!outId)return{ok:false,reason:'NO_REPLACEMENT_TARGET'};const result=VVPlayerState.returnFromLibero(team,liberoId,outId,{rallyNumber:VVPlayerState.rallyNumber});if(!result.ok)return result;const lp=allTeamObjects(team).find(p=>p.userData.playerState.id===liberoId),regular=allTeamObjects(team).find(p=>p.userData.playerState.id===outId);if(lp&&regular){regular.position.copy(lp.position);regular.userData.baseX=lp.userData.baseX;regular.userData.baseZ=lp.userData.baseZ;regular.userData.rotationPosition=result.rotationPosition}applyRosterChange(team);tip('LIBERO OUT • '+regular.name+' RETURNS');return result},
