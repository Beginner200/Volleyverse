...
let servingTeam='home', controlledIndex=0, controlled=null, paused=false, rallyLocked=false;const rotationState={home:0,away:0};const rotationSlots=[1,6,5,4,3,2];
const controlMode=()=>localStorage.getItem('volleyverseControlMode')||'team';
let homePlayers=[], awayPlayers=[], homeBenchPlayers=[], awayBenchPlayers=[], ball=null;
const COURT_LENGTH=18, COURT_WIDTH=9, HALF_LENGTH=9, HALF_WIDTH=4.5, ATTACK_LINE=3;
const ballState={v:new THREE.Vector3(),active:false,lastTouch:'home',cooldown:0,touches:0,lastAction:'',side:'home',targetX:0,targetZ:0,teamTouches:{home:0,away:0},crossed:false,rallyPhase:'SERVE'};
...
function updateHUD(){
  ...
  const rally=$('rallyInfo');
  if(rally){
    const phaseLabels={SERVE:'SERVE',IN_PLAY:'RALLY LIVE',NET_CONTACT:'NET CONTACT',POINT:'POINT'};
    const phase=phaseLabels[ballState.rallyPhase]||'READY';
    const homeTouches=ballState.teamTouches.home||0;
    const awayTouches=ballState.teamTouches.away||0;
    const serving=servingTeam==='home'?'VERSE':'RIVALS';
    rally.textContent=ballState.active
      ? phase+' • '+(ballState.side==='home'?'VERSE':'RIVALS')+' • TOUCHES '+homeTouches+'–'+awayTouches
      : phase+' • SERVE: '+serving;
  }
}
function updateRallyHUD(message){
  updateHUD();
...
function buildTeams(){
  ...
  controlled=homePlayers[0];applyRotationPositions('home',homePlayers);applyRotationPositions('away',awayPlayers);VVPlayerState.registerTeam('home',homePlayers.map(p=>p.userData.playerState));VVPlayerState.registerTeam('away',awayPlayers.map(p=>p.userData.playerState));homePlayers.forEach(p=>{p.userData.playerState.rotationPosition=p.userData.rotationPosition});awayPlayers.forEach(p=>{p.userData.playerState.rotationPosition=p.userData.rotationPosition});
  const homeBenchIds=[...(selectedLibero?[selectedLibero.id]:[]),...roster.map(c=>c.id).filter(id=>!data.some(c=>c.id===id)&&id!==(selectedLibero?.id||''))].slice(0,3);
  homeBenchPlayers=homeBenchIds.map((id,i)=>{const ch=roster.find(c=>c.id===id);const p=createPlayer(ch.color||0x36bfff,ch.name,5.8,6.8+i*.8,true,ch.position||'OH',matchStats(ch));p.userData.characterId=ch.id;p.userData.playerState.liberoEligible=ch.position==='L';p.visible=false;return p});
  const awayBenchIds=roster.map(c=>c.id).filter(id=>!awayIds.includes(id)&&id!==selectedLibero?.id).slice(0,3);
  awayBenchPlayers=awayBenchIds.map((id,i)=>{const ch=roster.find(c=>c.id===id);const p=createPlayer(ch.color||0xff4f79,ch.name,-5.8,6.8+i*.8,false,ch.position||'OH',matchStats(ch));p.userData.characterId=ch.id;p.userData.playerState.liberoEligible=ch.position==='L';p.visible=false;return p});
  VVPlayerState.registerBench('home',homeBenchPlayers.map(p=>p.userData.playerState));VVPlayerState.registerBench('away',awayBenchPlayers.map(p=>p.userData.playerState));
  const homeLibero=homeBenchPlayers.find(p=>p.userData.playerState.liberoEligible);if(homeLibero)VVPlayerState.setLibero('home',homeLibero.userData.playerState.id);
  const awayLibero=awayBenchPlayers.find(p=>p.userData.playerState.liberoEligible);if(awayLibero)VVPlayerState.setLibero('away',awayLibero.userData.playerState.id);
  VVPlayerState.setControlled(controlled.userData.playerState.id,true);
}
...
function allTeamObjects(team){return team==='home'?[...homePlayers,...homeBenchPlayers]:[...awayPlayers,...awayBenchPlayers]}
function swapRosterObjects(team,incoming,outgoing){
  const active=team==='home'?homePlayers:awayPlayers,bench=team==='home'?homeBenchPlayers:awayBenchPlayers;
  const ai=active.indexOf(outgoing),bi=bench.indexOf(incoming);
  if(ai>=0&&bi>=0){active[ai]=incoming;bench[bi]=outgoing}
}
function applyRosterChange(team){allTeamObjects(team).forEach(p=>{p.visible=p.userData.playerState.active;if(p.userData.playerState.active){p.userData.ring.visible=p===controlled;p.userData.arrow.visible=p===controlled}});if(team==='home'&&(!controlled||!controlled.userData.playerState.active)){controlledIndex=Math.max(0,homePlayers.findIndex(p=>p.userData.playerState.active));updateControlled()}}
window.VVSubstitutionAPI={
 substitute:(team,inId,outId)=>{if(ballState.active||!rallyLocked)return{ok:false,reason:'SUBSTITUTION_ONLY_BETWEEN_RALLIES'};const result=VVPlayerState.substitute(team,inId,outId,{rallyNumber:VVPlayerState.rallyNumber});if(!result.ok)return result;const incoming=allTeamObjects(team).find(p=>p.userData.playerState.id===inId),outgoing=allTeamObjects(team).find(p=>p.userData.playerState.id===outId);if(incoming&&outgoing){incoming.position.copy(outgoing.position);incoming.userData.baseX=outgoing.userData.baseX;incoming.userData.baseZ=outgoing.userData.baseZ;incoming.userData.rotationPosition=result.rotationPosition;swapRosterObjects(team,incoming,outgoing)}applyRosterChange(team);tip('SUBSTITUTION • '+result.incoming.name+' IN / '+result.outgoing.name+' OUT');return result},
 liberoIn:(team,liberoId,outId)=>{if(ballState.active||!rallyLocked)return{ok:false,reason:'LIBERO_ONLY_BETWEEN_RALLIES'};const result=VVPlayerState.replaceWithLibero(team,liberoId,outId,{rallyNumber:VVPlayerState.rallyNumber});if(!result.ok)return result;const libero=allTeamObjects(team).find(p=>p.userData.playerState.id===liberoId),outgoing=allTeamObjects(team).find(p=>p.userData.playerState.id===outId);if(libero&&outgoing){libero.position.copy(outgoing.position);libero.userData.baseX=outgoing.userData.baseX;libero.userData.baseZ=outgoing.userData.baseZ;libero.userData.rotationPosition=result.rotationPosition;swapRosterObjects(team,libero,outgoing)}applyRosterChange(team);tip('LIBERO IN • '+result.incoming.name+' FOR '+result.outgoing.name);return result},
 liberoOut:(team,liberoId)=>{if(ballState.active||!rallyLocked)return{ok:false,reason:'LIBERO_ONLY_BETWEEN_RALLIES'};const libero=VVPlayerState.get(liberoId),outId=libero?.lastReplacedId;if(!outId)return{ok:false,reason:'NO_REPLACEMENT_TARGET'};const result=VVPlayerState.returnFromLibero(team,liberoId,outId,{rallyNumber:VVPlayerState.rallyNumber});if(!result.ok)return result;const lp=allTeamObjects(team).find(p=>p.userData.playerState.id===liberoId),regular=allTeamObjects(team).find(p=>p.userData.playerState.id===outId);if(lp&&regular){regular.position.copy(lp.position);regular.userData.baseX=lp.userData.baseX;regular.userData.baseZ=lp.userData.baseZ;regular.userData.rotationPosition=result.rotationPosition;swapRosterObjects(team,regular,lp)}applyRosterChange(team);tip('LIBERO OUT • '+regular.name+' RETURNS');return result},
 state:()=>({rallyLocked,ballActive:ballState.active,home:VVPlayerState.getActive('home').map(p=>({id:p.id,name:p.name,role:p.role,rotationPosition:p.rotationPosition,isLibero:p.isLibero})),homeSubs:VVPlayerState.getSubstitutes('home').map(p=>({id:p.id,name:p.name,role:p.role,liberoEligible:p.liberoEligible,isLibero:p.isLibero})),used:VVPlayerState.substitutionsUsed})
};
...
function courtSlotPosition(team,position){
  ...
  const zBase={1:-3.45,6:-3.45,5:-3.45,4:-1.15,3:-1.15,2:-1.15}[position];
  return {x,z:team==='home'?zBase:-zBase};
}
function applyRotationPositions(team,players){
  players.forEach((p,i)=>{
...
function rotateTeam(team){
  rotationState[team]=(rotationState[team]+1)%6;
  applyRotationPositions(team,team==='home'?homePlayers:awayPlayers);
  const server=(team==='home'?homePlayers:awayPlayers).find(p=>p.userData.rotationPosition===1);
  if(server)tip((team==='home'?'HOME':'RIVALS')+' ROTATION • '+server.userData.name.toUpperCase()+' TO SERVE');
  return server;
}
...
function rallyFault(winner,reason){point(winner,reason);return false}
function validateContact(team,type){
  if(!ballState.active||rallyLocked)return false;
  const expectedSide=ball.position.z<0?'home':'away';
  if(type!=='block'&&team!==expectedSide){rallyFault(team==='home'?'away':'home','WRONG SIDE');return false}
  if(type!=='block'&&(ballState.teamTouches[team]||0)>=3){rallyFault(team==='home'?'away':'home','FOUR HITS');return false}
  return true;
}
function registerBallContact(team,type,{countTouch=true,cooldown=.25}={}){
  if(!validateContact(team,type))return false;
  if(!ballState.teamTouches||typeof ballState.teamTouches!=='object')ballState.teamTouches={home:0,away:0};
  if(countTouch){
    const nextTouch=(ballState.teamTouches[team]||0)+1;
    ballState.teamTouches[team]=nextTouch;
    ballState.touches=Math.max(0,ballState.touches||0)+1;
    ballState.rallyPhase=nextTouch===1?'RECEIVE':nextTouch===2?'SET':'ATTACK';
  }
...
function action(type){window.VVReplay?.record?.(type,{player:window.VVCharacters?.roster?.[controlledIndex]?.name||'PLAYER'});if(onlineMatchActive){const o=window.VVOnlineClient?.getLocalPlayer?.()||{};const slot=Number.isInteger(o.slot)?o.slot:onlineSlot;if(slot>=0)window.VVOnlineClient?.input?.(slot,keys.x,keys.z,type);tip('ONLINE • '+type.toUpperCase()+' SENT');return}
  ...
  if(!near||!ballApproaching||ball.position.y<.2||ball.position.y>4.4)return;if(type!=='serve'&&timing<.28){tip('TIMING OFF • WAIT FOR THE BALL');return;}const homeSide=ball.position.z<0;if(!homeSide&&type!=='block')return;if(type!=='block'&&!validateContact('home',type))return;
...
function updateApproach(dt){
  ...
  const u=controlled.userData;
  const speed=controlled.userData.speed||4.2;
  const mag=Math.hypot(u.moveX||0,u.moveZ||0);
  if(mag>.08&&state.ready&&!rallyLocked){
    controlled.position.x+=THREE.MathUtils.clamp(u.moveX,-1,1)*speed*dt;
    controlled.position.z+=THREE.MathUtils.clamp(u.moveZ,-1,1)*speed*dt;
  }
  const home=controlled.userData.home!==false;
  const minZ=home?-4.35:.25,maxZ=home?-.25:4.35;
  controlled.position.x=THREE.MathUtils.clamp(controlled.position.x,-4.25,4.25);
...
let last=performance.now();function startGameLoop(){if(state.rafId!==null)cancelAnimationFrame(state.rafId);state.running=true;last=performance.now();state.lastRender=0;state.frameErrors=0;state.rafId=requestAnimationFrame(loop)}function loop(now){state.rafId=requestAnimationFrame(loop);const dt=Math.min(Math.max((now-last)/1000,0),.025);last=now;if(state.ready){let frameFailed=false;try{physics(dt)}catch(err){frameFailed=true;console.error('VOLLEYVERSE physics error',err);tip('GAMEPLAY ERROR • '+String(err?.message||err).slice(0,70))}try{animatePlayers(dt)}catch(err){frameFailed=true;console.error('VOLLEYVERSE animation error',err)}try{if(onlineMatchActive&&window.VVOnlineClient?.isConnected?.()&&now-onlineLastInputAt>70){const o=window.VVOnlineClient.getLocalPlayer?.()||{};if(Number.isInteger(o.slot)){onlineSlot=o.slot;window.VVOnlineClient.input(o.slot,keys.x,keys.z,null);onlineLastInputAt=now}}if(localConnected()&&localHost())window.VVLocalMultiplayer.sendSnapshot(localNetSnapshot())}catch(err){console.error('VOLLEYVERSE network frame error',err)}try{const cameraPref=JSON.parse(localStorage.getItem('volleyverseCamera')||'{}');if(cameraPref.mode==='player'&&controlled){const playerPos=new THREE.Vector3();controlled.getWorldPosition(playerPos);const behind=controlled.userData.home?-1:1;cameraFollowTarget.set(playerPos.x+behind*6.5,3.25,playerPos.z);state.camera.position.lerp(cameraFollowTarget,.14);state.camera.lookAt(playerPos.x,playerPos.y+1.05,playerPos.z)}}catch(err){console.error('VOLLEYVERSE camera error',err)}if(now-state.lastRender>=state.renderEvery){try{state.renderer.render(state.scene,state.camera);state.lastRender=now}catch(err){frameFailed=true;console.error('VOLLEYVERSE renderer error',err);tip('GRAPHICS ERROR • '+String(err?.message||err).slice(0,70))}}if(!frameFailed)state.frameErrors=0;else state.frameErrors=Math.min(state.frameErrors+1,3)}}
...
$('playBtn')?.addEventListener('pointerdown',()=>{if($('playBtn').textContent==='PLAY AGAIN'){homeScore=awayScore=homeSets=awaySets=0;setNumber=1;matchHomePoints=0;servingTeam='home';rotationState.home=rotationState.away=0;controlledIndex=0;resetPlayers();updateControlled();updateHUD();$('overlayTitle').textContent='READY?';$('overlayText').textContent='Move your player, receive, set, spike and defend.';$('playBtn').textContent='PLAY MATCH';resetBall()}else{startSoloMatch()}});window.VVStartMatch=startSoloMatch;if(localStorage.getItem('volleyverseStartMatch')==='1'){localStorage.removeItem('volleyverseStartMatch');setTimeout(startSoloMatch,0)};function startSoloMatch(){try{if(state.rafId!==null)cancelAnimationFrame(state.rafId);window.VVLocalMultiplayer?.reset?.()}catch(e){}state.rafId=null;state.running=false;state.ready=false;state.lastRender=0;localStorage.setItem('volleyverseMatchMode','real');onlineMatchActive=false;onlineTeam='home';onlineSlot=-1;localSessionStarted=true;paused=false;rallyLocked=false;homeScore=awayScore=homeSets=awaySets=0;setNumber=1;matchHomePoints=0;servingTeam='home';controlledIndex=0;VVPlayerState.resetMatch();resetPlayers();updateControlled();$('overlay').classList.add('hidden');if(!createScene())return;state.ready=true;updateHUD();resetBall();startGameLoop();tip('READY • SERVE TO START THE RALLY')} 
