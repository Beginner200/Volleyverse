...
const state = { renderer:null, camera:null, scene:null, ready:false, running:false, mobile:false, renderEvery:16.7, lastRender:0, frameErrors:0, inputSendAt:0, aiAt:0, aiStep:1 };
const keys = {x:0,z:0};
...
let last=performance.now();function loop(now){requestAnimationFrame(loop);const dt=Math.min(Math.max((now-last)/1000,0),.025);last=now;if(state.ready){let frameFailed=false;try{physics(dt)}catch(err){frameFailed=true;console.error('VOLLEYVERSE physics error',err);tip('GAMEPLAY ERROR • '+String(err?.message||err).slice(0,70))}try{animatePlayers(dt)}catch(err){frameFailed=true;console.error('VOLLEYVERSE animation error',err)}try{if(onlineMatchActive&&window.VVOnlineClient?.isConnected?.()&&now-onlineLastInputAt>70){const o=window.VVOnlineClient.getLocalPlayer?.()||{};if(Number.isInteger(o.slot)){onlineSlot=o.slot;window.VVOnlineClient.input(o.slot,keys.x,keys.z,null);onlineLastInputAt=now}}if(localConnected()&&localHost())window.VVLocalMultiplayer.sendSnapshot(localNetSnapshot())}catch(err){console.error('VOLLEYVERSE network frame error',err)}try{if(localStorage.getItem('volleyverseCamera')==='player'&&controlled){cameraFollowTarget.set(controlled.position.x+5.2,4.4,controlled.position.z+6.4);state.camera.position.lerp(cameraFollowTarget,.08);state.camera.lookAt(controlled.position.x,1,controlled.position.z)}}catch(err){console.error('VOLLEYVERSE camera error',err)}if(now-state.lastRender>=state.renderEvery){try{state.renderer.render(state.scene,state.camera);state.lastRender=now}catch(err){frameFailed=true;console.error('VOLLEYVERSE renderer error',err);tip('GRAPHICS ERROR • '+String(err?.message||err).slice(0,70))}}if(!frameFailed)state.frameErrors=0;else state.frameErrors=Math.min(state.frameErrors+1,3)}}
