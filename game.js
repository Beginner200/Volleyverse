...
function action(type){window.VVReplay?.record?.(type,{player:window.VVCharacters?.roster?.[controlledIndex]?.name||'PLAYER'});
  ...
  if(!state.ready||rallyLocked)return;if(type==='serve'){serve();window.dispatchEvent(new CustomEvent('vv-skill',{detail:{type,player:controlled?.userData?.name||'PLAYER',position:controlled?.userData?.position||'',timing:1,quality:1}}));return}if(!ballState.active||ballState.cooldown>0)return;
  ...
  if(type!=='block')ballState.teamTouches.home++;ballState.lastTouch='home';ballState.side='home';ballState.lastAction=type;ballState.cooldown=.25;controlled.userData.action=type==='dive' ? .8 : .55;window.dispatchEvent(new CustomEvent('vv-skill',{detail:{type,player:controlled?.userData?.name||'PLAYER',position:controlled?.userData?.position||'',timing,quality:skillQuality(controlled,type,.9)}}));
}
...
let last=performance.now(),replayClock=0;function loop(now){const dt=Math.min((now-last)/1000,.035);last=now;if(state.ready){physics(dt);animatePlayers(dt);replayClock+=dt;if(replayClock>=.12){replayClock=0;window.VVReplay?.recordFrame?.({homeScore,awayScore,homeSets,awaySets,setNumber,servingTeam,ball:ball?{x:ball.position.x,y:ball.position.y,z:ball.position.z,active:ballState.active,lastTouch:ballState.lastTouch}:null,players:[...homePlayers,...awayPlayers].map(p=>({x:p.position.x,z:p.position.z,y:p.position.y,name:p.userData.name,action:p.userData.action}))});}if(localConnected()&&localHost())window.VVLocalMultiplayer.sendSnapshot(localNetSnapshot());state.renderer.render(state.scene,state.camera)}requestAnimationFrame(loop)}
