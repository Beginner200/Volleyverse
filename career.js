...
function addXP(amount){const s=load();const before=s.level;s.xp+=amount;while(s.xp>=need(s.level)&&s.level<100){s.xp-=need(s.level);s.level++}save(s);render();return {...s,xpAward:amount,leveledUp:s.level>before}}
...
function recordMatch({won,setsWon,points}){const s=load();s.wins+=won?1:0;s.sets+=Math.max(0,setsWon||0);s.points+=Math.max(0,points||0);const xp=won?100:40;const result=addXP(xp);return {...result,won,setsWon,points}}
...
function init(){document.getElementById('careerClose')?.addEventListener('click',close);document.getElementById('careerTrain')?.addEventListener('click',()=>{const s=load();s.training++;save(s);addXP(25)});render()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
return {load,save,addXP,recordMatch,open,close,rank};
})();
