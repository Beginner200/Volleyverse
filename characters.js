...
const VVCharacterCollection=(()=>{
  const key='volleyverseCollection';
  const rarityCost={Common:0,Uncommon:100,Rare:250,Epic:500,Legendary:900,Mythic:1600,Champion:3000};
  const load=()=>{try{return JSON.parse(localStorage.getItem(key)||'null')||{owned:['astra','kairo','nova','rex','mira','zen'],gems:500}}catch(e){return {owned:['astra','kairo','nova','rex','mira','zen'],gems:500}}};
  const save=s=>localStorage.setItem(key,JSON.stringify(s));
  const state=()=>{const s=load();s.owned=[...new Set(s.owned)];return s};
  const isOwned=id=>state().owned.includes(id);
  const unlock=id=>{const c=roster.find(x=>x.id===id),s=state();if(!c||isOwned(id))return {ok:true,state:s};const cost=rarityCost[c.rarity]??500;if(s.gems<cost)return {ok:false,reason:'NOT ENOUGH GEMS',cost,state:s};s.gems-=cost;s.owned.push(id);save(s);return {ok:true,cost,state:s}};
  const grantGems=n=>{const s=state();s.gems+=Math.max(0,Number(n)||0);save(s);return s};
  return {rarityCost,state,isOwned,unlock,grantGems};
})();
window.VVCharacterCollection=VVCharacterCollection;
...
function selectedCharacters(){
  const ids=selectedRoster()||['astra','kairo','nova','rex','mira','zen'];
  return ids.map(id=>roster.find(c=>c.id===id)).filter(Boolean);
}
function characterById(id){return roster.find(c=>c.id===id)||null}
window.VVCharacters={roster,positions,selectedRoster,saveRoster,selectedCharacters,characterById,openCharacterSelect};
...
const render=()=>{grid.innerHTML=roster.map(c=>`<button class="char-card ${rarityClass(c.rarity)} ${chosen.includes(c.id)?'chosen':''}" data-char="${c.id}"><div class="char-top"><span class="rarity">${c.rarity}</span><strong>${c.ovr}</strong></div><div class="char-visual" style="--accent:#${c.color.toString(16).padStart(6,'0')}"><img src="${window.VVCharacterArt?.dataUri(c)||''}" alt="${c.name} volleyball player" loading="lazy"></div><div class="char-name-row"><b>${c.name}</b><span>${c.gender}</span></div><span class="char-position">${c.position} • ${positions[c.position]}</span><small>${c.style}</small><div class="mini-stats"><span>SPIKE <b>${c.stats.spike}</b></span><span>SET <b>${c.stats.set}</b></span><span>RECV <b>${c.stats.receive}</b></span><span>BLOCK <b>${c.stats.block}</b></span></div></button>`).join('');grid.querySelectorAll('.char-card').forEach(card=>card.addEventListener('click',()=>{const id=card.dataset.char;if(!VVCharacterCollection.isOwned(id)){const result=VVCharacterCollection.unlock(id);if(!result.ok){document.getElementById('characterHint').textContent='NOT ENOUGH GEMS • NEED '+result.cost+' GEMS';return}document.getElementById('characterHint').textContent='UNLOCKED • '+id.toUpperCase();}if(chosen.includes(id)){if(chosen.length>1)chosen=chosen.filter(x=>x!==id)}else if(chosen.length<6)chosen.push(id);else{document.getElementById('characterHint').textContent='TEAM FULL • REMOVE A PLAYER TO ADD ANOTHER';return}document.getElementById('characterHint').textContent='SELECT 6 PLAYERS • 1 S + 2 OH/OPP + 2 MB + 1 L RECOMMENDED';render();renderTeam()}))};
const renderTeam=()=>{teamStrip.innerHTML=chosen.map((id,i)=>{const c=roster.find(x=>x.id===id);return `<div class="team-slot"><span>${i+1}</span><b>${c.name}</b><small>${c.position} • OVR ${c.ovr}</small><button data-remove="${id}">×</button></div>`}).join('');count.textContent=`${chosen.length}/6`;teamStrip.querySelectorAll('[data-remove]').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();if(chosen.length>1){chosen=chosen.filter(x=>x!==b.dataset.remove);render();renderTeam()}}));const chemistry=window.VVChemistry?.calculate(chosen,roster);
const chemEl=document.getElementById('chemistryScore');if(chemEl&&chemistry)chemEl.textContent=chemistry.score+'%';
const chemLabel=document.getElementById('chemistryLabel');if(chemLabel&&chemistry)chemLabel.textContent=chemistry.label;
const chemDetail=document.getElementById('chemistryDetail');if(chemDetail&&chemistry)chemDetail.textContent=chemistry.detail;
